...
import com.ayseozcan.util.data.Sample;
...
public class DataImpl implements ApplicationRunner {
    ...
    public void saveAllMoviesFromTvMazeToDb() {
        try {
            ...
            Arrays.asList(array).forEach(x -> {
                Movie movie = null;
                if (x.network == null) {
                    movie = Movie.builder().id(x.id)
                            .url(x.url)
                            .image(x.image.getMedium())
                            .language(x.language)
    ...
    public void createUser() {
        ...
        user4.setComments(List.of(
                ...
                MovieComment.builder().content("sahane").date(LocalDate.now().minusDays(12)).user(user4).movie(movieService.findbyId(105L)).build(),
